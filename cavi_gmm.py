...
def cavi(xobs:np.ndarray, kcat, sigma, rng:np.random.Generator=None, tol=1e-4, max_iter=1_000):
    if rng is None:
    ...
    while True:
        for i in range(n):
            phi_arr[i, :, iter] = update_cluster_assgn(kcat, mean_mat[:, iter-1], s2_mat[:, iter-1], xobs[i])
        for k in range(kcat):
            mean_mat[k, iter], s2_mat[k, iter] = update_center(sigma, phi_arr[:,:,iter], xobs, k)
        elbo_list[iter] = elbo(kcat, xobs, sigma, mean_mat[:, iter], s2_mat[:, iter], phi_arr[:, :, iter])
        if abs(elbo_list[iter] - elbo_list[iter - 1]) < tol:
            break
        iter += 1
    return CaviGmmResult(elbo_list[1:iter+1], mean_mat[:, 0:iter], s2_mat[:, 0:iter], phi_arr[:, :, 0:iter])
